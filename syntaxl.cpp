...
/*virtual*/ void SyntaxHighlighter::highlightBlock(const QString& str)
{
        ...
        if (nState == InsideCStyleComment) {
            ...
            if (str.mid(i, 1) == "\"" || str.mid(i, 1) == "\'") {
                if (str.mid(i - 1, 2) != "\\\""
                    ...
                    nState = NormalState;
                    setFormat(nStart, i - nStart + 1, Qt::darkGreen);
                }
            }
        }
        else {
            if (str.mid(i, 2) == "//") {
                setFormat(i, str.length() - i, Qt::darkRed);
                break;
            }
            else if (str.mid(i, 1) == "#") {
                ...
                if (!strKeyword.isEmpty()) {
                    setFormat(i, strKeyword.length(), Qt::darkRed);
    ...
    if (nState == InsideCString) {
        setFormat(nStart, str.length() - nStart, Qt::darkGreen);
    }
...
QString SyntaxHighlighter::getKeyword(int nPos, const QString& str) const
{
    ...
    foreach (QString strKeyword, m_listkeywords) {
